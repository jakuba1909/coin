...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBiotechAmount"))
        settings.setValue("nAnonymizeBiotechAmount", 1000);
    nAnonymizeBiotechAmount = settings.value("nAnonymizeBiotechAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBiotechAmount"))
        SoftSetArg("-anonymizeINSaNeamount", settings.value("nAnonymizeBiotechAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBiotechAmount:
            return QVariant(nAnonymizeBiotechAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBiotechAmount:
            nAnonymizeBiotechAmount = value.toInt();
            settings.setValue("nAnonymizeBiotechAmount", nAnonymizeBiotechAmount);
            emit AnonymizeBiotechAmountChanged(nAnonymizeBiotechAmount);
            break;
        default:
            break;
        }
