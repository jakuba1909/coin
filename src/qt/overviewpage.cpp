...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBiotechAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBiotechAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBiotechAmount = strAnonymizeBiotechAmount.remove(strAnonymizeBiotechAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBiotechAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBiotechAmount*COIN) nMaxToAnonymize = nAnonymizeBiotechAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBiotechAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBiotechAmount));
        strAnonymizeBiotechAmount = strAnonymizeBiotechAmount.remove(strAnonymizeBiotechAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBiotechAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
