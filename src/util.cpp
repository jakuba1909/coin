...
int nAnonymizeBiotechAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\biotech
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\biotech
    // Mac: ~/Library/Application Support/biotech
    // Unix: ~/.biotech
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "biotech";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "biotech.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "biotechd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Biotech will not work properly.");
